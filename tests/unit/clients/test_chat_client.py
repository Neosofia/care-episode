...
@patch("src.clients.chat_client.get_http_client")
def test_create_interaction_success(mock_get_client, mock_token_broker, _mock_resolve, flask_request_context):
...
@patch("src.clients.chat_client.token_broker")
@patch("src.clients.chat_client.get_http_client")
def test_create_interaction_upstream_500(mock_get_client, mock_token_broker, _mock_resolve, flask_request_context):
    mock_token_broker.return_value.get_token.return_value = "service-token"
    mock_client = MagicMock()
    mock_get_client.return_value = mock_client
    response = MagicMock()
...
@patch("src.clients.chat_client.resolve_service_base_url", return_value="http://chat:8019")
@patch("src.clients.chat_client.get_http_client")
def test_create_completion_network_error(mock_get_client, _mock_resolve, flask_request_context):
    mock_client = MagicMock()
    mock_get_client.return_value = mock_client
